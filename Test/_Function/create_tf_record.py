# -*- coding: utf-8 -*-
"""
Created on Wed Feb 13 17:07:25 2019
...
flags.DEFINE_string('annotations_json_dir', None, 
...
#os.path.relpath("P:/Roi/Python_model/24_CrossSectionImg/0_Mask-RCNN_Flow/W_plug")
#images_dir = "W_plug"
#annotations_json_dir = "W_plug"
#label_map_path = "path_to_label_map.pbtxt"
#output_path = "result.record"
...
def ImgCopy(annotation_json_path, output_path, DirN):  
    print("annotation_json_path=",annotation_json_path)
    ...
    images_dir = os.sep.join(annotation_json_path.split(os.sep)[:-1])
    image_path = os.path.join(images_dir, image_name)
    if not os.path.exists(image_path):
        return None
    TargetDir = output_path+os.sep+"Test"
    if not os.path.isdir(TargetDir): os.mkdir(TargetDir)
    img = cv2.imread(image_path,1)
    cv2.imwrite(TargetDir+os.sep+image_path.replace(os.sep,"_").replace(".","_")[:-4]+".bmp",img)
    #copy2(image_path,TargetDir+os.sep+image_path.replace(os.sep,"_").replace(".","_")[:-4]+".bmp")
    
    for mark in shapes:
        class_name = mark.get('label')
        if class_name == 'hole': continue
        else:
            DirN[class_name] = DirN.get(class_name,0)+1
    return DirN
    
def _get_annotation_dict(annotation_json_path, DirN):  
...
#        raise ValueError('`annotation_json_path` is not exist.')
    ...
    if image_relative_path is None:
        return None, None
    image_name = image_relative_path.split('//')[-1].split('\\')[-1]
    images_dir = os.sep.join(annotation_json_path.split(os.sep)[:-1])
    image_path = os.path.join(images_dir, image_name)
    image_format = image_name.split('.')[-1].replace('jpg', 'jpeg')
    
    image_path = [path for path in glob.glob(image_path[:-5]+'*') if path[-4:]!="json"][0]
...
def main(_):
    ...
    writer = []    
    for i in range(2):
        writer.append(tf.io.TFRecordWriter(FLAGS.output_path+os.sep+KindPath[i]+".record"))
    ...
    for i, annotation_file in enumerate(annotations_json_path):
        ...
        if ix==2: 
            DirNum[ix] = ImgCopy(annotation_file, FLAGS.output_path, DirNum[ix])
            continue
        ...
        annotation_dict, DirNum[ix] = _get_annotation_dict(annotation_file, DirNum[ix])
        if annotation_dict is None:
